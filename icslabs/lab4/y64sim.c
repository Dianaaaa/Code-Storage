...
stat_t nexti(y64sim_t *sim)
{
    ...
    int need_reg = 0;
    if (icode == I_RRMOVQ || icode == I_IRMOVQ || icode == I_MRMOVQ || icode == I_RMMOVQ || icode == I_ALU || icode == I_PUSHQ || icode == I_POPQ)
	need_reg = 1;
    int need_valc = 0;
    ...
    if (need_reg){    	
    	...
    	srcA = get_reg_val(sim->r, regA);
    	srcB = get_reg_val(sim->r, regB);
    }
	...
	addr = srcB + valC;
	set_long_val(sim->m, addr, srcA);
	sim->pc = next_pc;
	break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
	addr = srcB + valC;
	if (!get_long_val(sim->m, addr, &valM)){
	...
	valE = compute_alu(ifun, srcA, srcB);
	sim->cc = compute_cc(ifun, srcA, srcB, valE);
	...
	break;
      ...
      case I_CALL: /* 8:x imm */
	srcB = get_reg_val(sim->r, 4);
	valE = srcB - 8;	
	set_reg_val(sim->r, 4, valE);
	...
	break;
      case I_RET: /* 9:0 */
	srcA = get_reg_val(sim->r, 4);
	srcB = get_reg_val(sim->r, 4);
	valE = srcB + 8;
	if(!get_long_val(sim->m, srcA, &valM)){
	...
	valE = srcB - 8;
	set_reg_val(sim->r, 4, valE);
	if(!set_long_val(sim->m, valE, srcA)){
	    err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, valE);
	    return STAT_ADR;
	}	
	sim->pc = next_pc;
